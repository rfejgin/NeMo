...
def test_codec_latent_embedding_supports_selected_channels():
    model = _make_easy_magpie_model(_codec_latent_cfg({"frame_stacking_factor": 2}))
    codebook_indices = (2, 7, 13)
    codes = torch.stack(
        [_toy_codes(model, batch_size=2, num_frames=3)[:, codebook_idx // 2] for codebook_idx in codebook_indices],
        dim=1,
    )

    embedded = model.embed_audio_tokens(codes, codebook_indices=codebook_indices)
    expected = sum(
        model.embed_audio_codebook(codebook_idx, codes[:, input_idx])
...
@pytest.mark.parametrize("use_codec_latent_audio_embedding", [False, True], ids=["embedding_tables", "codec_latents"])
def test_acoustic_codes_predictor_process_batch_with_frame_stacking(use_codec_latent_audio_embedding):
    model = _make_easy_magpie_model(
        tiny_easy_magpie_cfg(
            {
                "use_codec_latent_audio_embedding": use_codec_latent_audio_embedding,
