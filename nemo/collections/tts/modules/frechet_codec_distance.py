...
if __name__ == "__main__":
    ...
    fcd = FrechetCodecDistance(codec=codec).to(device)

    audio_paths = [
        "/home/rfejgin/3368_170952_000004_000000.wav",
        "/home/rfejgin/6098_57837_000025_000000.wav",
    ]

    # build a batch of codes and codes_len
    codes_list = []
    codes_len_list = []
    max_len = -1
    for audio_path in audio_paths:
        codes, codes_len = fcd.encode_from_file(audio_path=audio_path)
        codes_list.append(codes)
        codes_len_list.append(codes_len)
        current_len = codes_len.cpu().item()
        if current_len > max_len:
            max_len = current_len
    B = len(audio_paths)
    C = codes.shape[1]
    codes_all = torch.zeros((B, C, max_len), device=device)
    for i, codes_len in enumerate(codes_len_list):
        codes_all[i, :, :codes_len] = codes_list[i]
    codes_len_all = torch.cat(codes_len_list)
    fcd.update(codes=codes_all, codes_len=codes_len_all, is_real=True)
    fcd.update(codes=codes_all, codes_len=codes_len_all, is_real=False)

    for audio_path in audio_paths:
        fcd.update_from_audio_file(audio_path=audio_path, is_real=True)
        fcd.update_from_audio_file(audio_path=audio_path, is_real=False)

    fcd_score = fcd.compute()
    print(fcd_score)

    # Todo
    # add a class that wraps the FID metric,
    # * implementing external APIs we need (update, update_from_audio_file, compute)
    # * pre-flattens codes before calling FID (since FID assumes number of examples is the first dimension)
    # * can also add a null feature extractor that does nothing
    # Note: does the codec care about context when dequantizing? or can we just flatten all batch * T  elements before the dequantize?
